...
let test_is_rule () =
  let test_strings = 
    ["animal(X) :- human(X)."; "hard :- subject(functional_programming)."] in
  List.iter (fun exp ->
...
let test_is_conj () =
  let test_strings = 
    ["mother(X, Y) :- parent(X, Y), woman(X)."; "mother(M, C) :- woman(M), parent(M, C)."] in
  List.iter (fun exp ->
...
let test_is_disj () =
  let test_strings = 
    ["pet(X) :- rat(X); cat(X); dog(X)."; "parent(X, Y) :- mother(X, Y); father(X, Y)."] in
  List.iter (fun exp ->
