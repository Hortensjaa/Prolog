...
let parse_clause exp =
  ...
  if (is_fact exp) then
    ...
    DRule(parse_with_state head, List.map (fun arg -> parse_with_state arg) args_list)
    ...
    CRule(parse_with_state head, List.rev_map (fun arg -> parse_with_state arg) args_list)
