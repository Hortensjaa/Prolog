...
let () = 
  ...
  let body = (Str.matched_group 2 exp) in 
  print_endline head;
  print_endline body;
  print_endline (string_of_bool (Parsing.Regex.Clauses.is_rule exp));
  print_endline (string_of_bool b);
  print_endline (string_of_bool (Parsing.Regex.Terms.is_compound head));
  print_endline (string_of_bool (Parsing.Regex.Terms.is_compound body));
  print_endline (string_of_bool (b && (Parsing.Regex.Terms.is_compound head) && (Parsing.Regex.Terms.is_compound body))); *)
